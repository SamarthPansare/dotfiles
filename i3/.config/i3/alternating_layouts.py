...
def set_layout(i3, e):
    ...
    # Skip if parent is tabbed or stacked
    if (parent and parent.layout != 'tabbed'
            and parent.layout != 'stacked'):

        # Also check grandparent to better respect manual layouts
        grandparent = find_parent(i3, parent.id) if parent else None
        if (grandparent and (grandparent.layout == 'tabbed'
                or grandparent.layout == 'stacked')):
            return
